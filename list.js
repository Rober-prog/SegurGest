...
export function renderList(){
  ...
  filtered.forEach(item=>{
    ...
    div.className = 'insurance-card col-6';
    ...
    if (item.type === 'Vehículo') {
      ...
      if (item.nextItv) assetDetailsHtml += `<div class="insurance-card-detail-item">Próxima ITV: <span class="insurance-card-detail-value">${formatDate(item.nextItv)}</span></div>`;
    ...
    div.innerHTML = `
      ...
      <div class="insurance-card-details-group">
        ...
        ${assetDetailsHtml}
        <div class="insurance-card-detail-item">Importe: <span class="insurance-card-detail-value price">${fmt(item.amount||0)} € (${item.paymentForm || '—'})</span></div>
